function [ output_args ] = sub_ET_observed(sufi2_in, observ_fold, begin_year,end_year, var_file_name, INPRINT)
...
for kk=1:length(var_file_name)
    ...
    fp = fopen(strcat(sufi2_in, 'observed_sub.txt'),format);
    if kk==1
        fprintf(fp,'%s\n', '23     : number of observed variables');
    end
    fprintf(fp,'\n');
